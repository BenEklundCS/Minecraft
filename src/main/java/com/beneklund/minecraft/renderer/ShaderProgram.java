    ...
    public ShaderProgram(String vertexShaderPath, String fragmentShaderPath) {
        try {
            ...
            try (InputStream vertexShaderStream = getClass().getResourceAsStream(vertexShaderPath)) {
                if (vertexShaderStream == null) { throw new IOException("Failed to load vertex shader from path: " + vertexShaderPath);  }
            ...
            try (InputStream fragmentShaderStream = getClass().getResourceAsStream(fragmentShaderPath)) {
                if (fragmentShaderStream == null) { throw new IOException("Failed to load fragment shader from path: " + fragmentShaderPath); }
