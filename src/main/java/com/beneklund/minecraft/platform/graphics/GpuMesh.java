...
public class GpuMesh {
    ...
    // Vertex format: 7 floats per vertex — x, y, z, u, v, ao, faceId. Stride = 28 bytes.
    public GpuMesh(float[] vertices, int[] indices) {
        ...
        vbo.upload(vertices);
        ebo.upload(indices);
        vao.attribPointer(0, 3, 28, 0L); // position: xyz
        vao.attribPointer(1, 2, 28, 12L); // uv
        vao.attribPointer(2, 1, 28, 20L); // ao
        vao.attribPointer(3, 1, 28, 24L); // faceId
        vao.unbind();
    }
